      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: Math.max(insets.bottom, 40) }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ backgroundColor: "#8B5E3C", borderRadius: 20, padding: 20, marginBottom: 16 }}>
          ...
          {b.averageRating > 0 && (
            <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginTop: 8 }}>
              ...
              <Text suppressHighlighting style={{ color: "#fff", fontWeight: "700", marginLeft: 4 }}>{b.averageRating.toFixed(1)} ({b.reviewsCount})</Text>
            </View>
          )}
          {b.description && <Text suppressHighlighting style={{ color: "#F5D5BA", marginTop: 10, fontSize: 14, lineHeight: 20 }}>{b.description}</Text>}
        </View>
          ...
          <View style={{ backgroundColor: "#fff", borderRadius: 20, padding: 16, marginBottom: 16, borderWidth: 1.5, borderColor: "#F0E8E0" }}>
            ...
            {services.length > 0 ? services.map((s: any, i: number) => (
              <View key={i} style={{ flexDirection: "row", justifyContent: "space-between", paddingVertical: 8, borderBottomWidth: i < services.length - 1 ? 1 : 0, borderColor: "#F0E8E0" }}>
                ...
                {s.price && <Text suppressHighlighting style={{ color: "#8B5E3C", fontWeight: "700", fontSize: 14 }}>€{s.price}</Text>}
              </View>
            )) : (
              <Text suppressHighlighting style={{ color: "#6B7280", fontSize: 14, lineHeight: 20 }}>{b.services}</Text>
            )}
          </View>
        )}
        ...
        <View style={{ backgroundColor: "#fff", borderRadius: 20, padding: 16, marginBottom: 16, borderWidth: 1.5, borderColor: "#F0E8E0" }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <Text suppressHighlighting style={{ fontWeight: "700", fontSize: 15, color: "#1A1A2E" }}>Avaliações ({reviews.length})</Text>
          ...
          {reviews.length === 0 ? (
            ...
            reviews.map((r: any) => (
              <View key={r.id} style={{ paddingVertical: 10, borderBottomWidth: 1, borderColor: "#F0E8E0" }}>
                ...
                {r.comment && <Text suppressHighlighting style={{ color: "#374151", fontSize: 13, marginTop: 4 }}>{r.comment}</Text>}
              </View>
            ))
          )}
        </View>
