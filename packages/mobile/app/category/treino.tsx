  ...
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.85} style={{
      ...
      <View style={{ flexDirection: "row", padding: 16, gap: 14 }}>
        <View style={{ width: 64, height: 64, borderRadius: 18, backgroundColor: "#ECFDF5", alignItems: "center", justifyContent: "center" }}>
...
export default function TreinoScreen() {
  ...
  const filtered = allBiz.filter((b: any) => b.type === "treino" &&
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#F0FDF4" }}>
      <CategoryHeader
        emoji="🎯"
        title={tr("Treino & Adestramento")}
        subtitle={tr("Treinadores profissionais para o seu pet")}
        bgColor="#ECFDF5"
        accentColor="#059669"
      />

      {/* Search */}
      <View style={{ marginHorizontal: 20, marginBottom: 16, flexDirection: "row", alignItems: "center", backgroundColor: "#fff", borderRadius: 16, borderWidth: 1.5, borderColor: "#D1FAE5", paddingHorizontal: 14, height: 46 }}>
        <Search size={18} color="#A0A0B0" />
        <TextInput
          value={search}
          onChangeText={setSearch}
          placeholder={tr("Pesquisar treinadores...")}
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 32 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        {isLoading ? (
          ...
          <View style={{ alignItems: "center", marginTop: 60 }}>
            ...
            <Text suppressHighlighting style={{ fontSize: 16, fontWeight: "700", color: "#1A1A2E", marginTop: 12 }}>{tr("Sem treinadores disponíveis")}</Text>
            <Text suppressHighlighting style={{ fontSize: 14, color: "#6B7280", marginTop: 6, textAlign: "center" }}>{tr("Ainda não há treinadores registados nesta área.")}</Text>
          </View>
        ) : (
          filtered.map((b: any) => (
