  ...
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.85} style={{
      ...
      <View style={{ flexDirection: "row", padding: 16, gap: 14 }}>
        <View style={{ width: 72, height: 72, borderRadius: 18, backgroundColor: "#FFFBEB", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
          {item.imageUrl ? (
        ...
        <View style={{ flex: 1 }}>
          <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <AlertCircle size={16} color="#F59E0B" style={{ marginLeft: 8 }} />
          </View>
          {item.location ? (
          ...
          {item.breed ? (
            <View style={{ marginTop: 6 }}>
              <View style={{ backgroundColor: "#FFFBEB", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8, alignSelf: "flex-start", borderWidth: 1, borderColor: "#FEF3C7" }}>
                <Text suppressHighlighting style={{ fontSize: 11, color: "#D97706", fontWeight: "700" }}>{item.breed}</Text>
              </View>
            </View>
          ) : null}
        </View>
...
export default function PerdidosScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#FFFDF0" }}>
      ...
      {/* Search */}
      <View style={{ marginHorizontal: 20, marginBottom: 16, flexDirection: "row", alignItems: "center", backgroundColor: "#fff", borderRadius: 16, borderWidth: 1.5, borderColor: "#FEF3C7", paddingHorizontal: 14, height: 46 }}>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 32 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        {isLoading ? (
          ...
          filtered.map((item: any) => (
            <LostCard key={item.id} item={item} onPress={() => router.push(`/listing/${item.id}`)} />
          ))
        )}
      </ScrollView>
    </SafeAreaView>
